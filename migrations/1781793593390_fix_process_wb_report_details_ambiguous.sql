-- ============================================================================
-- Migration: fix_process_wb_report_details_ambiguous
-- Description: Fix ambiguous column references in unmatched_sample
-- ============================================================================
...
BEGIN
    ...
    IF v_unmatched_count > 0 THEN
        SELECT COALESCE(jsonb_agg(
            jsonb_build_object(
                'rrd_id', sub.rrd_id,
                'doc_type_name', sub.doc_type_name,
                'seller_oper_name', sub.seller_oper_name,
                'bonus_type_name', sub.bonus_type_name,
                'for_pay', sub.for_pay,
                'delivery_service', sub.delivery_service,
                'penalty', sub.penalty
            )
