...
-- Description: Show only report_type = 1 in view
-- ============================================================================
...
-- Create updated view with filter by report_type = 1
...
WHERE h.report_type = 1  -- Only report_type = 1
ORDER BY h.report_id DESC;

COMMENT ON VIEW wb_fi_report_summary_view IS 'Report summary with date_from and date_to from headers, only report_type = 1';
...
-- BEGIN;
-- DROP VIEW IF EXISTS wb_fi_report_summary_view CASCADE;
-- CREATE OR REPLACE VIEW wb_fi_report_summary_view AS ... (previous version without filter);
-- COMMIT;