-- ============================================================================
-- Migration: update_storage_distribution_to_products
-- Description: Calculate storage for product summary proportionally by volume with correction
-- ============================================================================

BEGIN;

-- 1. Функция для расчёта storage по товарам (пропорционально объёму с коррекцией)
...
DECLARE
    ...
    v_storage_total NUMERIC(15,2) := 0;
BEGIN
    ...
    -- 4. Calculate total storage for summary (already calculated via rules)
    SELECT COALESCE(storage, 0) INTO v_storage_total
    FROM wb_fi_report_summary
    WHERE user_id = p_user_id AND report_id = p_summary_report_id;
    
    -- 5. Update summary table (seller_tax, overheads, logistics_total)
    ...
    -- 6. Update product overheads (proportionally by volume with correction)
    PERFORM calculate_product_overheads(p_user_id, p_summary_report_id);
    
    -- 7. Update product storage (proportionally by volume with correction)
    PERFORM calculate_product_storage(p_user_id, p_summary_report_id);
END;
$$;

COMMENT ON FUNCTION update_wb_complex_fields(INTEGER, BIGINT[], BIGINT) IS 
'Update complex fields including overheads and storage distribution to products';
