-- ============================================================================
-- Migration: add_calculate_report_totals_function
-- Description: Calculate report_totals as last step after all other calculations
-- ============================================================================

BEGIN;

-- 1. Функция для расчёта report_totals для общего summary
...
-- 2. Функция для расчёта report_totals для product_summary (обновляет таблицу)
CREATE OR REPLACE FUNCTION update_product_report_totals(
    p_user_id INTEGER,
    p_report_id BIGINT
)
...
-- 3. Обновить update_wb_complex_fields (убрать report_totals из неё)
...
BEGIN
    ...
    -- Update summary table (without report_totals)
...
-- 4. Обновить process_wb_report_details (вызывать report_totals последним)
...
BEGIN
    ...
    -- 7. Calculate all fields using wb_sum_by_product_rule
    FOR v_field IN 
        SELECT 
    ...
    -- 9. Update summary with total quantity, volume and cost_price
    SELECT * INTO v_qv 
    ...
    v_total_cost_price := COALESCE(v_qv.total_cost_price, 0);
    
    UPDATE wb_fi_report_summary
    SET 
        quantity = v_total_quantity,
        volume = v_total_volume,
        cost_price = v_total_cost_price
    WHERE user_id = p_user_id AND report_id = v_combined_report_id;
